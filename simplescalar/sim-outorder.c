...
/* JWHUR FMT global cycle counter */
static counter_t iL1_miss_cycle = 0;
static counter_t iL2_miss_cycle = 0;
static counter_t iTlb_miss_cycle = 0;
static counter_t dL1_miss_cycle = 0;
static counter_t dL2_miss_cycle = 0;
static counter_t dTlb_miss_cycle = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  /* JWHUR FMT stats */
  stat_reg_counter(sdb, "iL1_miss_cycle", "wasted cycles by iL1 cache miss", &iL1_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "iL2_miss_cycle", "wasted cycles by iL2 cache miss", &iL2_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "iTlb_miss_cycle", "wasted cycles by iTlb miss", &iTlb_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "dL1_miss_cycle", "wasted cycles by dL1 cache miss", &dL1_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "dL2_miss_cycle", "wasted cycles by dL2 cache miss", &dL2_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "dTlb_miss_cycle", "wasted cyclees by dTlb miss", &dTlb_miss_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "branch_mis_cycle", "wasted cycles by branch misprediction", &branch_mis_cycle, /* initial value */0, /* format */ NULL);
  stat_reg_counter(sdb, "longlat_fu_cycle", "wasted cycles by long latency functional unit", &longlat_fu_cycle, /* initial value */0, /* format */ NULL);
...
/* JWHUR FMT define */
struct FMT_row {
  INST_SEQ_TYPE ROB_ID;
  int mispredicted;

  counter_t branch_penalty;
  counter_t iL1_miss;
  counter_t iL2_miss;
  counter_t iTlb_miss;
};
...
{
  FMT_size = RUU_size;
  FMT = calloc(FMT_size, sizeof(struct FMT_row));
  if (!FMT)
    fatal("out of virtual memory");

  /* FMT_num : number of dispatched branches in FMT
FMT_fetch : pointer to the fetched element in FMT
FMT_dispatch_head : pointer to the oldest element in FMT
FMT_dispatch_tail : pointer next to the latest element in FMT */
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
  {
    ...
    /* JWHUR find branch and retire FMT element */
    if (MD_OP_FLAGS(rs->op) & F_CTRL) {
      struct FMT_row *fmt;
      fmt = &FMT[FMT_dispatch_head];

      if (fmt->ROB_ID != rs->seq) {
        myfprintf(stderr, "JWHUR ROB_ID: %d, rs->seq: %d\n", fmt->ROB_ID, rs->seq);
        panic("FMT ROB not synchronized!");
      }

      if (fmt->mispredicted) {
        branch_mis_cycle += fmt->branch_penalty;
        last_br_spec = FALSE;
      }
      else {
        iL1_miss_cycle += fmt->iL1_miss;
        iL2_miss_cycle += fmt->iL2_miss;
        iTlb_miss_cycle += fmt->iTlb_miss;
        last_br_spec = TRUE;
      }

      memset(&FMT[FMT_dispatch_head], 0, sizeof(struct FMT_row));
      FMT_dispatch_head = (FMT_dispatch_head + 1) % FMT_size;
      FMT_num--;
      myfprintf(stderr, "JWHUR Commit head: %d, tail: %d, num: %d\n", FMT_dispatch_head, FMT_dispatch_tail, FMT_num);
    }
...
ruu_writeback(void)
{
  ...
  while ((rs = eventq_next_event()))
  {
    ...
    if (rs->recover_inst)
    {
      /* JWHUR set FMT mispredict bit */
      struct FMT_row *fmt;
      int FMT_recover_tail, FMT_recovery, FMT_index;
      for (i=0; i<FMT_num; i++) {
        fmt = &FMT[(FMT_dispatch_head + i) % FMT_size];
        if (!fmt) 
          panic("FMT table not allocated!");

        if (fmt->ROB_ID == rs->seq) {
          FMT_index = FMT_dispatch_head + i;
          FMT_recover_tail = (FMT_dispatch_head + i + 1) % FMT_size;
          FMT_recovery = FMT_num - i - 1;
          fmt->mispredicted = TRUE;
          break;
        }
      }
      ...
      if (fmt->ROB_ID != rs->seq) {
        for (i=0; i<FMT_num; i++) {
          fmt = &FMT[(FMT_dispatch_head + i) % FMT_num];
          myfprintf(stderr, "FMT index: %d,ROB_ID: %d\n", (FMT_dispatch_head+i) % FMT_size, fmt->ROB_ID);
        }
        panic("branch not caught by FMT!");
      }

      /* JWHUR FMT refresh */
      for (i=0; i<FMT_recovery; i++)
        memset(&FMT[(FMT_index + i + 1) % FMT_size], 0, sizeof(struct FMT_row));

      FMT_fetch = FMT_recover_tail;
      FMT_dispatch_tail = FMT_recover_tail;
      FMT_num -= FMT_recovery;
      myfprintf(stderr, "JWHUR Writeback head: %d, tail: %d, num: %d\n", FMT_dispatch_head, FMT_dispatch_tail, FMT_num);
...
#include "machine.def"
    ...
    if (MD_OP_FLAGS(op) & F_CTRL)
    {
      ...
      /* JWHUR branch instruction dispatched, advance dispatch tail pointer */
      struct FMT_row *fmt;
      fmt = &FMT[FMT_dispatch_tail];
      fmt->ROB_ID = inst_seq;
      FMT_dispatch_tail = (FMT_dispatch_tail + 1) % FMT_size;
      FMT_num++;
      myfprintf(stderr, "JWHUR Dispatch head: %d, tail: %d, num: %d\n", FMT_dispatch_head, FMT_dispatch_tail, FMT_num);
    }
      ...
      i++)
  {
    ...
    if (pred)
    {
      ...
      if (MD_OP_FLAGS(op) & F_CTRL) {
        ...
        /* JWHUR if branch (F_CTRL) instruction is fetched and decoded,
           increment FMT fetch pointer and initialize */
        memset(&FMT[FMT_fetch], 0, sizeof(struct FMT_row));
...
/* JWHUR update FMT branch penalty */
  static void
FMT_update(void)
{
  int i;
...
/* JWHUR update FMT global counter */
  static void
FMT_counter_update(void) 
{

  if (!last_br_spec && !RUU_num)
    branch_mis_cycle++;

  if (RUU_num == RUU_size) { /* ROB buffer full */
    if (RUU[RUU_head].ea_comp && (MD_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD)) {
      switch (data_missed) {
        case dL1:
          dL1_miss_cycle++;
          break;
...
#include "machine.def"
  ...
  for (;;)
  {
    ...
    if (!ruu_fetch_issue_delay)
      ...
      /* JWHUR update FMT local inst miss counter */
      struct FMT_row *fmt = &FMT[FMT_fetch - 1];
      switch (inst_missed) {
        case iL1:
          fmt->iL1_miss++;
          break;
        ...
        case iTlb:
          ...
          break;
        default:
          panic("No source of instruction miss!");
      } 
      ruu_fetch_issue_delay--;
    }

    /* JWHUR FMT branch_penalty update */
    FMT_update();

    /* JWHUR FMT global counter update, global branch penalty increment if ROB empty
       due to branch misprediction, dL1, dL2, dTlb increment if ROB full due to them */
    FMT_counter_update();
